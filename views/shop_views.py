...
class ShopModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        loading_embed = discord.Embed(
            title=t("shop_loading", self.lang),
    ...
    async def _handle_shop(self, interaction: discord.Interaction, auth: AuthResult, region: str) -> None:
        ...
        if not data:
            error_embed = discord.Embed(description=t('shop_error_no_data', self.lang))
            await interaction.edit_original_response(embed=error_embed)
            return
        ...
        if len(embeds) <= 1:
            error_embed = discord.Embed(description=t('shop_no_skins', self.lang))
            await interaction.edit_original_response(embed=error_embed)
        else:
    ...
    async def _handle_nightmarket(self, interaction: discord.Interaction, auth: AuthResult, region: str) -> None:
        ...
        if not data:
            error_embed = discord.Embed(description=t('nm_error_no_data', self.lang))
            await interaction.edit_original_response(embed=error_embed)
            return
        ...
        if len(embeds) <= 1:
            error_embed = discord.Embed(description=t('nm_no_skins', self.lang))
            await interaction.edit_original_response(embed=error_embed)
        else:
            await interaction.edit_original_response(embeds=embeds)
...
class ShopView(BaseView):
    def __init__(self, api: ValorantAPI, auth_url: str, context: Union[discord.Interaction, commands.Context], mode: str = "shop", lang: str = DEFAULT_LANG) -> None:
        ...
        self.auth_btn = discord.ui.Button(label=t("btn_paste", lang), style=discord.ButtonStyle.primary)
